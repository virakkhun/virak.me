...
const UnsplashPage: NextPage = () => {
	const { data: UnsplashData } = trpc.GetUnsplashInfoAPI.useQuery()
	const [isAmbient, setIsAmbient] = useState(false)

	if (!UnsplashData) return <Loading text='Loading...' />

	const data = UnsplashData.data

	return (
		<>
			<Head>
				<title>{`${data?.name}'s Unsplash`}</title>
				...
				<div className='flex justify-between items-center mb-2'>
					...
					<div className='flex items-center gap-2'>
						...
						<button
							...
							className='w-8 h-4 rounded-full dark:bg-default bg-lightDefault relative focus:outline focus:outline-secondary/30'
						>
							<div
